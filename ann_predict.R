...
data <- read.csv("Gempa_Tsunami_NOAA_Bathy.csv")
...
train.aba <- cbind(data[, 3:ncol(data)])
...
# Compute predictions
predicted.nn <- neuralnet::compute(nn, train.aba[,1:13])
...
original_values <- max.col(train.aba[, 14])
...
for(i in 1:k){
  ...
  # Compute predictions
  predicted.nn <- neuralnet::compute(cross.nn, cross.test[, 1:13])
  
  # Extract results
  result.predicted.nn <- predicted.nn$net.result
  
  # Accuracy (test set)
  original_values <- max.col(cross.test[, 14])
  result.predicted.nn_2 <- max.col(result.predicted.nn)
...
results <- data.frame(actual = cross.test[14], prediction = predicted.nn$net.result)
